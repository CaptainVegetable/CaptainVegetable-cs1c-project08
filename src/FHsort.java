...
public class FHsort
{
    ...
    // Note: all our sorts will go into class FHsort as static generics
    ...
    void percolateDown(E[] a, int hole, int arraySize)
    {
        ...
        // client array starts at index 0
        // so we adjust how we calculate left and right child
        for( tmp = a[hole]; 2 * hole + 1 < arraySize; hole = child )
        {
            child = 2 * hole + 1;
            // if 2 children, get the GREATER of the two (because MAX heap)
...
// (a[center] has what used to be in a[right-1])
    ...
    public static void setRecursionLimit(int qsRecursionLimit) {
        QS_RECURSION_LIMIT = qsRecursionLimit;
    }
    ...
    // private insertionSort that works on sub-arrays --------------
    protected static < E extends Comparable< ? super E > >
    void insertionSort(E[] a, int start, int stop)
    {
        int k, pos;
