...
public class RecursionLimitTest {
    ...
    public static String[] testQuickSort(int testSize)
    {
        int k, randomInt;
        long startTime, estimatedTime;
        Integer[] arrayOfInts = new Integer[testSize];
        String[] outputStrArray = new String[2];
        ...
        outputStrArray[0] = Integer.toString(testSize);
        outputStrArray[1] = Long.toString(estimatedTime);
        ...
        try {
            ...
            for (int test = 0; test < ARRAY_SIZES.length; test++) {
                ...
                for (int recLimit = LOWER_LIMIT; recLimit <= UPPER_LIMIT; recLimit += LIMIT_STEP) {
                    ...
                    System.out.printf("For data size %s and recusion limit %s:\nRuntime = %s \n\n",
                            result[0], recursionLimit, result[1]);
                }
